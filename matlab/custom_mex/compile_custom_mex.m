function compile_custom_mex(varargin)
...
idx = strfind(root,filesep);
root = root(1:idx(end)-1);
...
if isequal(arch,'maci64')
  if ~opts.enableGpu
    if exist([prefdir '/mex_C++_maci64.xml'],'file')
      delete([prefdir '/mex_C++_maci64.xml']);
    end
    % In this case we enforce to use gcc instead of Xcode clang
    mex_opts = {['-setup:' prefdir '/mex_G++_maci64.xml'],'C++'};
    mex(mex_opts{:})
  else
    if exist([prefdir '/mex_C++_maci64.xml'],'file')
      delete([prefdir '/mex_C++_maci64.xml']);
...
% GPU-specific files
if opts.enableGpu
  mex_src{end+1} = fullfile(root,'matlab','custom_mex','src','FMapNLSum_gpu.cu');
  mex_src{end+1} = fullfile(root,'matlab','custom_mex','src','FMapNLSumT_gpu.cu');
  mex_src{end+1} = fullfile(root,'matlab','custom_mex','src','DerFMapNLSumT_gpu.cu');
end

if ~opts.enableGpu
  % CPU-specific files
  mex_src{end+1} = fullfile(root,'matlab','custom_mex','src','FMapNLSum_cpu.cpp');
  mex_src{end+1} = fullfile(root,'matlab','custom_mex','src','FMapNLSumT_cpu.cpp');
  mex_src{end+1} = fullfile(root,'matlab','custom_mex','src','DerFMapNLSumT_cpu.cpp');
end
...
% Build directories
mex_dir = fullfile(root, 'matlab', 'custom_mex', arch);
if opts.enableGpu
  bld_dir = fullfile(mex_dir, '.build');
  if ~exist(bld_dir, 'dir')
    mkdir(bld_dir);
  end
else
  bld_dir = mex_dir;
...
flags.mexcc{end+1} = sprintf('-I"%s"', fullfile(root, 'matlab','custom_mex','include'));                    
...
flags.nvcc = horzcat(flags.cc, ...
                     ...
                     {sprintf('-I"%s"', fullfile(root, 'matlab','custom_mex','include'))}, ...
...
% --------------------------------------------------------------------
str = fullfile('matlab','custom_mex','src') ;
...
if opts.enableGpu
  ...
  mopts = {'-outdir', fileparts(tgt), src, mex_opts{:}};
end
