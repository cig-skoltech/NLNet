function [y,dh,dg] = nn_cpgcft(x,h,wh,weights,g,idx,dzdy,varargin)

% NN_PGCFT (Transpose of Color Patch Group Collaborative Filtering)
%
...
%   extracted patches. 
%
%   Z = NN_CPGCFT(Y, H, WH, Weights, G, idx)
%
...
%
%   [DZDX,DH,DG] = NN_CPGCFT(X, H, WH, G, idx, DZDY) 
...
assert(size(wh,2) == size(h,4), ['nn_cpgcft:: Invalid input for ' ...
  'wh - dimensions mismatch.']);

assert( isempty(weights) || all(size(weights) == size(idx)), ...
  'nn_cpgcft:: Invalid input for weights - dimensions mismatch.');
...
if numel(g) ~= opts.Nbrs && numel(g) ~= opts.Nbrs*numFilters
  error(['nn_cpgcft:: The number of the neighbor-filter coefficients ' ...
...
if (isempty(dzdy) || nargin < 7)    
  ...
  if opts.conserveMemory       
    ...
    if useGPU
      for ch = 1:Nc
        ...
        y(:,:,idx_ch,:) = FMapNLSumT_gpu(x(:,:,idx_ch,:),weights,idx-1,n,I-1);
      end
    else
      for ch = 1:Nc
        idx_ch = (1:numFilters)+(ch-1)*numFilters;
        y(:,:,idx_ch,:) = FMapNLSumT_cpu(x(:,:,idx_ch,:),weights,idx-1,n,I-1);
      end      
    end
    ...
    y = cell(2,1);
    y{1} = zeros(size(x),'like',x);
    
    if useGPU
      for ch = 1:Nc
        idx_ch = (1:numFilters)+(ch-1)*numFilters;
        y{1}(:,:,idx_ch,:) = FMapNLSumT_gpu(x(:,:,idx_ch,:),weights,idx-1,n,I-1);
      end      
    else
      for ch = 1:Nc
        idx_ch = (1:numFilters)+(ch-1)*numFilters;
        y{1}(:,:,idx_ch,:) = FMapNLSumT_cpu(x(:,:,idx_ch,:),weights,idx-1,n,I-1);
      end      
    end
    
    if Nc > 1
      y{1} = reshape(y{1},patchDims(1),patchDims(2),[],Nc*NI);
    end
    ...
    y{2} = y{2}{2};    
    if Nc > 1
      y{2} = reshape(y{2},size(y{2},1),size(y{2},2),Nc,NI);
    end
  ...
  if (size(idx,1)~=patchDims(1) || size(idx,2)~=patchDims(2) || ...
      size(idx,3)~=opts.Nbrs || size(idx,4)~=NI)
    error('nn_cpgcft:: idx does not have the correct dimensions.');
  end
  
  if Nc > 1
    dzdy = reshape(dzdy,size(dzdy,1),size(dzdy,2),1,Nc*NI);
  end  
  ...
  if Nc > 1
    dzdy = reshape(dzdy,patchDims(1),patchDims(2),numFilters*Nc,NI);
  end
  ...
  if opts.derParams(2)
    ...
    elseif ~FMapSharedWeights && PatchSharedWeights
      for k=1:opts.Nbrs
        ...
        for ch = 1:Nc
          idx_ch = (1:numFilters)+(ch-1)*numFilters;
          tmp = DerFMapNLSumT(x{2}(:,:,idx_ch,:),dzdy(:,:,idx_ch,:),widx(:,k,:)-1, ...
            n(:,k,:),I(:,k,:)-1);          
          dg(sind) = dg(sind) + (sum(sum(reshape(tmp,[],numFilters,NI),1),3))';
        end
      end
    else
      for k=1:opts.Nbrs
        ...
        for ch = 1:Nc
          idx_ch = (1:numFilters)+(ch-1)*numFilters;
          tmp = DerFMapNLSumT(bsxfun(@times,x{2}(:,:,idx_ch,:),weights(:,:,k,:)),...
            dzdy(:,:,idx_ch,:),widx(:,k,:)-1,n(:,k,:),I(:,k,:)-1);         
          dg(sind) = dg(sind) + (sum(sum(reshape(tmp,[],numFilters,NI),1),3))';
        end
      end
    end
  ...
  if useGPU
    for ch = 1:Nc
      idx_ch = (1:numFilters)+(ch-1)*numFilters;
      y(:,:,idx_ch,:) = FMapNLSum_gpu(dzdy(:,:,idx_ch,:),weights,idx-1);
    end
  else
    for ch = 1:Nc
      idx_ch = (1:numFilters)+(ch-1)*numFilters;
      y(:,:,idx_ch,:) = FMapNLSum_cpu(dzdy(:,:,idx_ch,:),weights,idx-1);
    end    
  end
