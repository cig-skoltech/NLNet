function [y,dzdwh,dzdwht,dzdg,dzdgt,dzdw,dzdl,J,M] = nlcf(x,Obs,h,wh,wht,...
  g,gt,rbf_weights,rbf_means,rbf_precision,lambda,dzdy,varargin)
%(Non-Local Collarborative Filtering) 
%
% Each nlcf stage consists of the following layers:
% 1) NN_PGCF 2) NN_CLIP 3) NN_SHRINK_LUT 4) NN_PGCFT 5) NN_DATAFIDELITY
%
...
%
% 1) NN_PAD 2) NN_PGCF 3) NN_CLIP 4) NN_SHRINK_LUT 5) NN_PGCFT 
% 6) NN_PADT 7) NN_DATAFIDELITY
%
%  If any of wht and gt are empty then the 2nd and 5th layers share the  
%  same parameter wh or/and g.
%
%   Y = NLCF(X,Obs,H,WH,WHT,G,GT,RBF_WEIGHTS,MEANS,PRECISION,LAMBDA)
%
...
%
%   [DZDX, DZDWH, DZDWHT, DZDG, DZDGT, DZDW, DZDL] = UDNLCF(X, Obs, H, ...
...
%
%   NLCF(...,'OPT',VALUE,...) takes the following options:
%
...
% [xe,~,~,~,~,~,~,J,M] = nlcf(y,y,h,wh,[],g,[],rbf_weights,rbf_means,...
...
% [dzdx,dzdwh,~,dzdg,~,dzdw,dzdl] = nlcf({y,{xe{3},xe{2}},xe{1}},...
...
switch opts.shrink_type
  ...
  otherwise
    error('nlcf :: Unknown type of RBF shrinkage.');
end
...
if isempty(opts.Nbrs) 
  if isempty(opts.Nbrs_idx)
    error('nlcf :: The number of closest neighbors must be specified.');
  else
    opts.Nbrs = size(opts.Nbrs_idx,3);
  end
end

assert(size(wh,1) == size(rbf_weights,1), ['Invalid input for ' ...
...
assert( numel(g) == opts.Nbrs || numel(g) == opts.Nbrs*size(wh,1), ...
...
if nargin < 12 || isempty(dzdy)
  ...
  % Block-matching
  if isempty(opts.Nbrs_idx)
    [opts.Nbrs_idx,opts.Nbrs_weights] = misc.patchMatch(x,'patchSize', ...
  ...
  if opts.conserveMemory
    y = nn_pgcf(x,h,wh,opts.Nbrs_weights,g,opts.Nbrs_idx,[],'stride',...
    ...
    if nargout > 7
      [y,~,J] = Shrink(y,rbf_weights,rbf_means,rbf_precision,[],...
    ...
    if weightSharing && GroupWeightSharing
      y = nn_pgcft(y,h,wh,opts.Nbrs_weights,g,opts.Nbrs_idx,[],'stride',...
        opts.stride,'Nbrs',opts.Nbrs,'cuDNN',opts.cuDNN,'conserveMemory',true);
    elseif weightSharing && ~GroupWeightSharing
      y = nn_pgcft(y,h,wh,opts.Nbrs_weights,gt,opts.Nbrs_idx,[],'stride',...
        opts.stride,'Nbrs',opts.Nbrs,'cuDNN',opts.cuDNN,'conserveMemory',true);      
    elseif ~weightSharing && GroupWeightSharing
      y = nn_pgcft(y,h,wht,opts.Nbrs_weights,g,opts.Nbrs_idx,[],'stride',...
        opts.stride,'Nbrs',opts.Nbrs,'cuDNN',opts.cuDNN,'conserveMemory',true);            
    else
      y = nn_pgcft(y,h,wht,opts.Nbrs_weights,gt,opts.Nbrs_idx,[],'stride',...
        opts.stride,'Nbrs',opts.Nbrs,'cuDNN',opts.cuDNN,'conserveMemory',true);            
    end
    ...
    y{1} = nn_pgcf(x,h,wh,opts.Nbrs_weights,g,opts.Nbrs_idx,[],'stride',...
   ...
   if nargout > 8
    ...
    if nargout > 7
      [y{2},~,J] = Shrink(y{1}{3},rbf_weights,rbf_means,...
        rbf_precision,[],'Idx',opts.Idx,'data_mu',opts.data_mu,'step',...
        opts.step,'origin',opts.origin);
    else
      y{2} = Shrink(y{1}{3},rbf_weights,rbf_means,...
        rbf_precision,[],'Idx',opts.Idx,'data_mu',opts.data_mu,'step',...
        opts.step,'origin',opts.origin);
    end
    
    if weightSharing && GroupWeightSharing
      y{3} = nn_pgcft(y{2},h,wh,opts.Nbrs_weights,g,opts.Nbrs_idx,[],'stride',...
        opts.stride,'Nbrs',opts.Nbrs,'cuDNN',opts.cuDNN,'conserveMemory',false);
    elseif weightSharing && ~GroupWeightSharing
      y{3} = nn_pgcft(y{2},h,wh,opts.Nbrs_weights,gt,opts.Nbrs_idx,[],'stride',...
        opts.stride,'Nbrs',opts.Nbrs,'cuDNN',opts.cuDNN,'conserveMemory',false);      
    elseif ~weightSharing && GroupWeightSharing
      y{3} = nn_pgcft(y{2},h,wht,opts.Nbrs_weights,g,opts.Nbrs_idx,[],'stride',...
        opts.stride,'Nbrs',opts.Nbrs,'cuDNN',opts.cuDNN,'conserveMemory',false);            
    else
      y{3} = nn_pgcft(y{2},h,wht,opts.Nbrs_weights,gt,opts.Nbrs_idx,[],'stride',...
        opts.stride,'Nbrs',opts.Nbrs,'cuDNN',opts.cuDNN,'conserveMemory',false);            
    end
  ...
  if weightSharing && GroupWeightSharing
    [y,dzdwht,dzdgt] = nn_pgcft(x{2},h,wh,opts.Nbrs_weights,g, ...
      opts.Nbrs_idx,y,'stride',opts.stride,'cuDNN',opts.cuDNN, ...
      'Nbrs',opts.Nbrs,'derParams',logical(opts.learningRate([1,3])));
  elseif weightSharing && ~GroupWeightSharing
    [y,dzdwht,dzdgt] = nn_pgcft(x{2},h,wh,opts.Nbrs_weights,gt, ...
      opts.Nbrs_idx,y,'stride',opts.stride,'cuDNN',opts.cuDNN, ...
      'Nbrs',opts.Nbrs,'derParams',logical(opts.learningRate([1,4])));    
  elseif ~weightSharing && GroupWeightSharing
    [y,dzdwht,dzdgt] = nn_pgcft(x{2},h,wht,opts.Nbrs_weights,g, ...
      opts.Nbrs_idx,y,'stride',opts.stride,'cuDNN',opts.cuDNN, ...
      'Nbrs',opts.Nbrs,'derParams',logical(opts.learningRate([2,3])));
  else
    [y,dzdwht,dzdgt] = nn_pgcft(x{2},h,wht,opts.Nbrs_weights,gt, ...
      opts.Nbrs_idx,y,'stride',opts.stride,'cuDNN',opts.cuDNN, ...
      'Nbrs',opts.Nbrs,'derParams',logical(opts.learningRate([2,4])));    
  end
  x{2} = [];
    
  [y, dzdw] = Shrink(x{3}{3},rbf_weights,rbf_means,rbf_precision,y,...
  ...
  [y,dzdwh,dzdg] = nn_pgcf(x{3}(2:-1:1),h,wh,opts.Nbrs_weights,g, ...
    opts.Nbrs_idx,y,'stride',opts.stride,'cuDNN',opts.cuDNN, ...
    'Nbrs',opts.Nbrs,'derParams',logical(opts.learningRate([1,3])));
