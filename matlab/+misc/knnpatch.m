function [knn,knn_D,LUT]=knnpatch(f,blocksize,searchwin,varargin)
...
% specific patch number stored in the array P=im2patch_cpu(f,blocksize,stride);
%
% f=double(imread('peppers.png'));f=f/max(f(:));
% blocksize=[8 8];stride=[3,2]; pad=[0, 0, 0, 0];searchwin=[10 10];K=10;
% [knn,LUT]=misc.knnpatch(f,blocksize,searchwin,'stride',stride,'K',K);
% 
% %% ***  Look Up Table ***
...
% %% ***  ***  ***
% 
% P=im2patch_gpu(f,blocksize,stride,pad);
...
% fpatch=f(i-floor((blocksize(1)-1)/2):i+ceil((blocksize(1)-1)/2),j-floor((blocksize(2)-1)/2):j+ceil((blocksize(2)-1)/2),:);
% % Verify the correctness of the mapping
% reshape(P(LUT(sub),:,:),[blocksize size(f,3)])-fpatch
...
% f=double(imread('peppers.png'));f=f/max(f(:));
% blocksize=[8 8];stride=[1,1]; searchwin=[15 15];K=16;
% [knn,LUT]=knnpatch(f,blocksize,searchwin,'stride',stride,'K',K);
% P=im2patch(f,blocksize,stride);
% idx1=randsrc(1,1,1:size(knn,1)); % choose randomly a patch.
% V=P(LUT(knn(idx1,:,1)),:,:,);
% V=reshape(V,[blocksize size(f,3) K]);
% J=vl_imarray(V);
% imshow(J/255,[]);
% % or 
...
if size(opts.W)~=blocksize
  error('knnpatch:InvalidInput','W must be a vector of size equal to blocksize.');
end
...
patch_dims= max(floor(([Nx Ny]-blocksize)./opts.stride)+1,0);
...
tmp=tmp(end:-1:2);
sx=[tmp 0:opts.stride(1):searchwin(1)];
tmp=0:-opts.stride(2):-searchwin(2);
tmp=tmp(end:-1:2);
sy=[tmp 0:opts.stride(2):searchwin(2)];
...
for kx=sx
  for ky=sy
    ...
    if ~(kx==0 && ky==0) % We do not check the (0,0)-offset case since in
      ...
      %D=vl_nnconv(E,repmat(opts.W,[1 1 1 Nc]),[],'stride',opts.stride,opts.cuDNN);
      %D=sum(D,3);
      D=vl_nnconv(E,repmat(opts.W,[1 1 Nc]),[],'stride',opts.stride,opts.cuDNN);
