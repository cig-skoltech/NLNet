function res = nlcf_eval(net, x, dzdy, res, varargin)
%NLCF_EVAL  Evaluate the network.
%
...
%
%   NLCF layer:
%     The NLCF layer wraps nlcf(). It has fields:
%
%     - `layer.type` contains the string `'nlcf'.
    ...
    case 'nlcf'
      ...
      if doder
        [res(i+1).x,~,~,~,~,~,~,res(i+1).aux{1},res(i+1).aux{2}] = ...
          nlcf(checkInput(res(i).x), opts.netParams.Obs, l.filters{1}, ...
        ...
        res(i+1).x = nlcf(checkInput(res(i).x), opts.netParams.Obs, ...
    ...
    switch l.type
      ...
      case 'nlcf'
        ...
        [res(i).dzdx,dzdw{1},dzdw{2},dzdw{3},dzdw{4},dzdw{5},dzdw{6}] = ...
          nlcf(input, opts.netParams.Obs, l.filters{1}, l.weights{1}, ...
